...
import overviewImg from './starter-code/assets/planet-saturn.svg';
import structureImg from './starter-code/assets/planet-saturn-internal.svg';
import surfaceImg from './starter-code/assets/geology-saturn.png';
import sourceIcon from './starter-code/assets/icon-source.svg'

const saturnData = Data.filter(info => info.name === "Saturn")
    ...
    return (
        ...
        {saturnData.map(data => (
            ...
            <div className="flex flex-row justify-between font-spartans text-9 font-bold px-6 border-b  border-gray-300 md:flex-col md:w-281 md:h-154 md:border-b-0 md:relative md:top-100 md:ml-auto lg:w-350 lg:top-96.5">
                ...
                <button onClick={handleOverview} className={`md:p-4 md:border md:border-gray-300 text-left lg:hover:bg-slate-600  ${overview ? 'border-b-4 border-saturnTheme h-10 md:bg-saturnTheme': ''}`} >
                    <span className=" hidden md:inline-block md:mr-4">01</span>
                    OVERVIEW
                </button>

                <button onClick={handleStructure} className={`md:p-4 md:border md:border-gray-300 text-left lg:hover:bg-slate-600 ${structure ? 'border-b-4 border-saturnTheme h-10 md:bg-saturnTheme': ''}`} > 
                ...
                <button onClick={handleSurface} className={`md:p-4 md:border md:border-gray-300 text-left lg:hover:bg-slate-600 ${surface ? 'border-b-4 border-saturnTheme h-10 md:bg-saturnTheme': ''}`} >
            ...
            {overview ? (<article className="flex flex-col justify-center items-center"> 
                <img src={overviewImg} alt="earth" className="w-64 h-64 my-12 md:w-422 md:h-422 md:mt-0 lg:w-416 lg:h-416 lg:mr-96"/> 
                <div className="text-center md:text-left md:w-339 md:h-64 md:mr-auto md:px-6 md:-mt-10 lg:ml-auto lg:mr-0 lg:relative lg:bottom-96 lg:-mt-40">
                    <h2 className="text-40 font-antonio mb-4 md:text-5xl lg:text-8xl">SATURN</h2> 
            ...
            : structure ? (<article className="flex flex-col justify-center items-center">
                <img src={structureImg} alt="earth" className="w-64 h-64 my-12 md:w-422 md:h-422 md:mt-0 lg:w-416 lg:h-416 lg:mr-96"/> 
                <div className="text-center md:text-left md:w-339 md:h-64 md:mr-auto md:px-6 md:-mt-10 lg:ml-auto lg:mr-0 lg:relative lg:bottom-96 lg:-mt-40">
                    <h2 className="text-40 font-antonio mb-4 md:text-5xl lg:text-8xl ">SATURN</h2> 
                    <p className="font-spartans text-xs leading-22 px-6 my-4 md:px-0 md:my-10 lg:text-sm">{data.geology.content}</p>
                    <p className="text-xs leading-6 text-gray-400 lg:text-sm">
            ...
            : (<article className="flex flex-col justify-center items-center"> 
                <img src={overviewImg} alt="earth" className="w-64 h-64 mt-12 md:w-422 md:h-422 md:mt-0 lg:w-416 lg:h-416 lg:mr-96"/> 
                <img src={surfaceImg} alt="earth" className="relative bottom-24 w-24 h-24 -mb-12 md:w-28 md:h-28 md:bottom-32 lg:w-40 lg:h-40 lg:right-48 lg:-mt-4"/> 
                <div className="text-center md:text-left md:w-339 md:h-64 md:mr-auto md:px-6 md:-mt-14 lg:ml-auto lg:mr-0 lg:relative lg:bottom-96 lg:-mt-52">
                    <h2 className="text-40 font-antonio mb-4 md:text-5xl lg:text-8xl">SATURN</h2> 
                    <p className="font-spartans text-xs leading-22 px-6 my-4 md:px-0 md:my-10 lg:text-sm">{data.geology.content}</p>
                    <p className="text-xs leading-6 text-gray-400 lg:text-sm">
                        Source: <span className="underline text-white">Wikipedia</span>
