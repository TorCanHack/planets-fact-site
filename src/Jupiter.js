...
import overviewImg from './starter-code/assets/planet-jupiter.svg';
import structureImg from './starter-code/assets/planet-jupiter-internal.svg';
import surfaceImg from './starter-code/assets/geology-jupiter.png';
import sourceIcon from './starter-code/assets/icon-source.svg'

const jupiterData = Data.filter(info => info.name === "Jupiter")
...
const Jupiter = () => {
    ...
    return (
        <>
        {jupiterData.map(data => (
            <section key={data.name} className="bg-[url('/src/starter-code/assets/background-stars.svg')] lg:px-52">
             <div className="flex flex-row justify-between font-spartans text-9 font-bold px-6 border-b  border-gray-300 md:flex-col md:w-281 md:h-154 md:border-b-0 md:relative md:top-100 md:ml-auto lg:w-350 lg:top-96.5">

                <button onClick={handleOverview} className={`md:p-4 md:border md:border-gray-300 text-left lg:hover:bg-slate-600  ${overview ? 'border-b-4 border-jupiterTheme h-10 md:bg-jupiterTheme ': ''}`} >
                    <span className=" hidden md:inline-block md:mr-4">01</span>
                    OVERVIEW
                </button>

                <button onClick={handleStructure} className={`md:p-4 md:border md:border-gray-300 text-left lg:hover:bg-slate-600 ${structure ? 'border-b-4 border-jupiterTheme  h-10 md:bg-jupiterTheme ': ''}`} > 
                ...
                <button onClick={handleSurface} className={`md:p-4 md:border md:border-gray-300 text-left lg:hover:bg-slate-600 ${surface ? 'border-b-4 border-jupiterTheme  h-10 md:bg-jupiterTheme ': ''}`} >
            ...
            {overview ? (<article className="flex flex-col justify-center items-center"> 
                <img src={overviewImg} alt="earth" className="w-56 h-56 my-20 md:w-369 md:h-369 md:mt-0 lg:w-582 lg:h-582 lg:mr-96 lg:mb-56"/> 
                <div className="text-center md:text-left md:w-339 md:h-64 md:mr-auto md:px-6 md:-mt-10 lg:ml-auto lg:mr-0 lg:relative lg:bottom-96 lg:-mt-97">
                    <h2 className="text-40 font-antonio mb-4 md:text-5xl lg:text-8xl">JUPITER</h2> 
            ...
            : structure ? (<article className="flex flex-col justify-center items-center">
                <img src={structureImg} alt="earth" className="w-56 h-56 my-20 md:w-369 md:h-369 md:mt-0 lg:w-582 lg:h-582 lg:mr-96 lg:mb-56"/> 
                <div className="text-center md:text-left md:w-339 md:h-64 md:mr-auto md:px-6 md:-mt-10 lg:ml-auto lg:mr-0 lg:relative lg:bottom-96 lg:-mt-97">
                    <h2 className="text-40 font-antonio mb-4 md:text-5xl lg:text-8xl">JUPITER</h2> 
                    <p className="font-spartans text-xs leading-22 px-6 my-4 md:px-0 md:my-10 lg:text-sm">{data.overview.content}</p>
                    <p className="text-xs leading-6 text-gray-400 lg:text-sm">
            ...
            : (<article  className="flex flex-col justify-center items-center">
                <img src={overviewImg} alt="earth" className="w-56 h-56 mt-20 md:w-369 md:h-369 md:mt-0 lg:w-582 lg:h-582 lg:mr-96 lg:mb-56"/> 
                <img src={surfaceImg} alt="earth" className="relative bottom-10 w-24 h-24 -mb-4 md:w-32 md:h-32 md:bottom-12 md:-mt-2 lg:w-40 lg:h-40 lg:right-48 lg:bottom-96"/> 
                <div className="text-center md:text-left md:w-339 md:h-64 md:mr-auto md:px-6 md:-mt-16 lg:ml-auto lg:mr-0 lg:relative lg:bottom-96 lg:-mt-98">
                    <h2 className="text-40 font-antonio mb-4 md:text-5xl lg:text-8xl">JUPITER</h2> 
                    <p className="font-spartans text-xs leading-22 px-6 my-4 md:px-0 md:my-10 lg:text-sm">{data.overview.content}</p>
                    <p className="text-xs leading-6 text-gray-400 lg:text-sm">
                        Source: <span className="underline text-white">Wikipedia</span>
            ...
            <article className="px-6 my-4 md:flex md:flex-row md:justify-between lg:mt-16 ">
