...
import overviewImg from './starter-code/assets/planet-uranus.svg';
import structureImg from './starter-code/assets/planet-uranus-internal.svg';
import surfaceImg from './starter-code/assets/geology-uranus.png';
import sourceIcon from './starter-code/assets/icon-source.svg'

const uranusData = Data.filter(info => info.name === "Uranus")
...
const Uranus = () => {
    ...
    return (
        ...
        {uranusData.map(data => (
            ...
            <div className="flex flex-row justify-between font-spartans text-9 font-bold px-6 border-b  border-gray-300 md:flex-col md:w-281 md:h-154 md:border-b-0 md:relative md:top-99 md:ml-auto lg:w-350 lg:top-96.5 ">

                <button onClick={handleOverview} className={`md:p-4 md:border md:border-gray-300 text-left lg:hover:bg-slate-600  ${overview ? 'border-b-4 border-uranusTheme h-10 md:bg-uranusTheme': ''}`} >
                    <span className=" hidden md:inline-block md:mr-4">01</span>
                    OVERVIEW
                </button>

            <button onClick={handleStructure} className={`md:p-4 md:border md:border-gray-300 text-left lg:hover:bg-slate-600  ${structure ? 'border-b-4 border-uranusTheme h-10 md:bg-uranusTheme': ''}`} > 
            ...
            <button onClick={handleSurface} className={`md:p-4 md:border md:border-gray-300 text-left lg:hover:bg-slate-600  ${surface ? 'border-b-4 border-uranusTheme h-10 md:bg-uranusTheme': ''}`} >
            ...
            {overview ? (<article className="flex flex-col justify-center items-center"> 
                <img src={overviewImg} alt="Uranus" className="w-44 h-44 my-20 md:w-72 md:h-72 md:mt-0 lg:w-450 lg:h-450 lg:mr-96"/> 
                <div className="text-center md:text-left md:w-339 md:h-64 md:mr-auto md:px-6 lg:ml-auto lg:mr-0 lg:relative lg:bottom-96 lg:-mt-56">
                    <h2 className="text-40 font-antonio mb-4 md:text-5xl lg:text-8xl">URANUS</h2> 
            ...
            : structure ? (<article className="flex flex-col justify-center items-center">
                <img src={structureImg} alt="Uranus" className="w-44 h-44 my-20 md:w-72 md:h-72 md:mt-0 lg:w-450 lg:h-450 lg:mr-96"/> 
                <div className="text-center md:text-left md:w-339 md:h-64 md:mr-auto md:px-6 lg:ml-auto lg:mr-0 lg:relative lg:bottom-96 lg:-mt-56">
                    <h2 className="text-40 font-antonio mb-4 md:text-5xl lg:text-8xl">URANUS</h2> 
                    <p className="font-spartans text-xs leading-22 px-6 my-4 md:px-0 md:my-10 lg:text-sm">{data.overview.content}</p>
                    <p className="text-xs leading-6 text-gray-400 lg:text-sm">
            ...
            : (<article  className="flex flex-col justify-center items-center">
                <img src={overviewImg} alt="Uranus" className="w-44 h-44 mt-20 md:w-72 md:h-72 md:mt-0 lg:w-450 lg:h-450 lg:mr-96"/>
                <img src={surfaceImg} alt="Uranus" className="relative bottom-10 w-20 h-20 mb-0 md:w-28 md:h-28 lg:w-40 lg:h-40 lg:right-48 lg:bottom-20 lg:-mt-4"/> 
                <div className="text-center md:text-left md:w-339 md:h-64 md:mr-auto md:px-6 md:-mt-8 lg:ml-auto lg:mr-0 lg:relative lg:bottom-96 lg:-mt-72">
                    <h2 className="text-40 font-antonio mb-4 md:text-5xl lg:text-8xl">URANUS</h2> 
                    <p className="font-spartans text-xs leading-22 px-6 my-4 md:px-0 md:my-10 lg:text-sm">{data.overview.content}</p>
                    <p className="text-xs leading-6 text-gray-400 lg:text-sm">
                        Source: <span className="underline text-white">Wikipedia</span>
